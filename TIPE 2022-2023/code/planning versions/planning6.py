...
def setup():
    ...
    ajouter_f = Frame(fenetre,bg=color_bg)
    ...
    menuDeroulant = Menubutton(deroulant, text='Menu', width='20', borderwidth=2, bg='gray', activebackground=color_bts,relief = RAISED, font=("Helvetica", "10", "bold"))
    menuDeroulant.grid()
...
def ajouter_event():
    ...
    infos = (count1, Titreb.get(), Debutb.get(), Finb.get(), Categorieb.get(), False, Descriptionb.get(), salleget())
    cursor.execute(req,infos)
    cnx.commit()
    ajouter_personne2()
    for i,a in enumerate(coche):
        if i!=6:
            if a.get()==1:
                req = "INSERT INTO PARTICIPE_GROUPES (ParticipeID_g, EventID_ext, ClasseID_ext) VALUES (%s, %s, %s)"
                infos = (countPG, count1, i+1)
                cursor.execute(req,infos)
                cnx.commit()
                countPG+=1
        else:
...
def Ajouter_cours():
    ...
    if count ==0:
        ...
        for i,a in enumerate(A):
            ...
            var_name=IntVar()
            coche+=[var_name]
            Checkbutton(groupes_event,text=a, variable = var_name,onvalue = 1, offvalue = 0,).place(x=15,y=20*i)
        global Titre
...
def planning_ds():
    ...
    menuDeroulant = Menubutton(Menu_planning, text='Menu', width='20', borderwidth=2, bg='gray', activebackground=color_bts,relief = RAISED)
    menuDeroulant.grid()
    menuDeroulant1 = Menu(menuDeroulant)
...
def create_day_boxes(parent,now):
    ...
    for i in range(nb_jours):   
        ...
        button = Button(day_box, text="{} {}".format(day_name, i+1),font=('Arial',11), activebackground=color_bg,anchor="n", command=lambda day=i+1: show_day_events(day, events), background=color,width=20,height=7)
...
def eventsDetails(event):
    ...
    event_details_window.geometry("400x500+550+300")
...
# background color
code_color_r_1, code_color_g_1, code_color_b_1 = 200,203,250
...
setup2()
Co.geometry("+600+350")
Co.config(bg=color_bg,cursor="X_cursor")
Co.mainloop()
if lancer1 :
    ...
    fenetre.config(bg=color_bg,cursor="X_cursor")
    fenetreHeight = 950
    fenetreWidth = 1800
    ...
    labelMonth = Label(fenetre, text=dt.format("MMMM YYYY"),font=("Helvetica", 20,'bold'), background=color_bg, foreground='#000088')
    ...
    days_frame.grid(row=0, column=0, padx=fenetreWidth/2-350, pady=fenetreHeight/2-450)
