...
class button_frame:
    def __init__(self,frame,nom,command,pos,size):
        posx,posy=pos
        sizex,sizey=size
        self.button=Button(frame,text=nom,command=command, background=color_bts,font=("Helvetica", 10,'bold'),width=sizex,height=sizey)
...
def eventsDetails(event):
    ...
    event_label = Label(event_details_window, text='Date de début: '+ str(event[3])[0:-3],font=('Arial',11, BOLD),bg=color_bg)
    event_label.place(x=10,y=40)
    event_label = Label(event_details_window, text='Date de fin: '+str(event[3])[0:-3],font=('Arial',11, BOLD),bg=color_bg)
...
if lancer1 :
    ...
    refresh_button = button_frame(frame1, "Refresh", refresh, (20,20),(15,1))
    change_color_button = button_frame(frame1, "", change_color, (20,fenetreHeight-200),(1,1))
