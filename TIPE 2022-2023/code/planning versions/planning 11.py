...
def setup():
    ...
    # entries(frame2,(50,100), 19)
    entries(frame2,(50,100), 14)
...
def create_entry_widgets(parent, font_size):
    ...
    category_combo = create_combobox(parent, ['Ds', 'partiels', 'jour férié', 'conseil de classe', 'autre'], 1, 4)
    room_entry = create_entry_with_label(parent, 'Salle', 5, font_size)
    description_entry = create_entry_with_label(parent, 'Description', 6, font_size)

    return title_entry, start_entry, end_entry, category_combo, room_entry, description_entry

def create_entry_with_label(parent, text, column, font_size):
...
def create_combobox(parent, values, row, column):
    combo_var = StringVar()
    combo = ttk.Combobox(parent, textvariable=combo_var, values=values, state='readonly')
...
""" def affichage_nom_colonnes_entries():
    ...
    if count == 0:
        ...
        group_cb = create_combobox(frame2, group_names, 2, 3)

        salle_check = IntVar()
        salle_check_btn = Checkbutton(frame2, text='salle réservée ', variable=salle_check, onvalue=1, offvalue=0, bg=color_bg)
        salle_check_btn.grid(row=2, column=4)

        personnes = Label(frame2, text='ajouter une personne',)
        personnes.grid(row=3, column=0)
        personnesb = Entry(frame2)
        personnesb.grid(row=4, column=0)

        ajout_personne_button = Button(frame2, text='ajouter la personne', command=ajouter_personne)
        ajout_personne_button.grid(row=5, column=0)

        ajout_event_button = Button(frame2, text="ajouter l'événement", command=insert_event_in_bdd, bg=color_bts)
        ajout_event_button.grid(row=2, column=0) """

def entries(window, pos, font_size):
    global selected_groupe, count, personnesb, salle_check

    posx, posy = pos
    entry_frame = Frame(window, bg=color_bg)
    entry_frame.grid(row=10, column=10, padx=posx, pady=posy)

    

    if window != frame2:
        global Titre, Debut, Fin, Categorie, Salle, Description
        Titre, Debut, Fin, Categorie, Salle, Description = create_entry_widgets(entry_frame, font_size)
    else:
        global Titre2, Debut2, Fin2, Categorie2, Salle2, Description2
        global Categorie2, Categorie2_combobox
        Titre2, Debut2, Fin2, Categorie2, Salle2, Description2 = create_entry_widgets(entry_frame, font_size)
        if count == 0:
            count = 1
            ...
            group_names = cursor.fetchall()
            selected_groupe = StringVar()
            group_cb = create_combobox(entry_frame, group_names, 1, 3)

            salle_check = IntVar()
            salle_check_btn = Checkbutton(entry_frame, text='salle réservée ', variable=salle_check, onvalue=1, offvalue=0, bg=color_bg)
            salle_check_btn.grid(row=2, column=4)

            """ personnes = Label(entry_frame, text='ajouter une personne',)
            personnes.grid(row=3, column=0)
            personnesb = Entry(entry_frame)
            personnesb.grid(row=4, column=0)

            ajout_personne_button = Button(entry_frame, text='ajouter la personne', command=ajouter_personne)
            ajout_personne_button.grid(row=5, column=0) """

            ajout_event_button = Button(entry_frame, text="ajouter l'événement", command=insert_event_in_bdd, bg=color_bts)
            ajout_event_button.grid(row=10, column=10)
        """ Categorie2 = Label(entry_frame, text='Categorie', bg=color_bg, font=('ARIAL', 19, BOLD))
        Categorie2.grid(row=4, column=1)
        Categorie2_combobox = ttk.Combobox(entry_frame)
        Categorie2_combobox['values'] = ['Ds', 'partiels', 'jour férié', 'conseil de classe', 'autre']
        Categorie2_combobox['state'] = 'readonly'
        Categorie2_combobox.grid(row=4, column=2) """

        group_label = Label(entry_frame, text='Groupe:', bg=color_bg, font=('ARIAL', font_size, BOLD))
        ...
        add_person_label.grid(row=4, column=1)

        add_person_entry = Entry(entry_frame)
        add_person_entry.grid(row=5, column=1)

        Button(entry_frame, text='Ajouter la personne', command=ajouter_personne).grid(row=6, column=1)
...
def modifierEvent(eventID):
    ...
    if ((Titre.entry.get()=='') or (Titre.entry.get()==' ')):
        ...
        C=Titre.entry.get()
    if ((Categorie2_combobox.get() == '') or (Categorie2_combobox.get() == ' ')):
        D = liste[0][4]
    else:
        D = Categorie2_combobox.get()
