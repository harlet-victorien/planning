...
def tableau_cpg1():
    ...
    tableau = ttk.Treeview(fenetre_planning, columns=('Date','Horaire','Titre','Salle'),show = 'headings',selectmode='browse')
    vsb = ttk.Scrollbar(fenetre_planning,orient="vertical",command=tableau.yview)
    ...
    vsb.place(x=20,y=50,height=200)
...
def ajouter_event():
    ...
    infos = (count1, Titreb.get(), Debutb.get(), Finb.get(), Categorieb.get(), salle_check.get(), Descriptionb.get(), salleget())
    cursor.execute(req,infos)
    cnx.commit()
...
def planning_ds():
    ...
    menuDeroulant1 = Menu(menuDeroulant)
    if 1 in persClasses:
        menuDeroulant1.add_command(label='CPG2', command = tableau_cpg2)
    if 2 in persClasses:
...
def modifierEvent(eventID):
    ...
    print(liste)
    if ((Debutb2.get()=='') or (Debutb2.get()==' ')):
        A=liste[0][2]
    else:
        A=Debutb2.get()

    if ((Finb2.get()=='') or (Finb2.get()==' ')):
        B=liste[0][3]
    else:
        B=Finb2.get()
    if ((Titreb2.get()=='') or (Titreb2.get()==' ')):
        C=liste[0][1]
    else:
        C=Titreb2.get()
    if ((Categorieb2.get()=='') or (Categorieb2.get()==' ')):
        D=liste[0][4]
    else:
        D=Categorieb2.get()
    if ((Descriptionb2.get()=='') or (Descriptionb2.get()==' ')):
        E=liste[0][6]
    else:
        E=Descriptionb2.get()
    id=str(eventID)
...
if lancer1 :
    ...
    car = ttk.Frame(days_frame,width=20, height=20, border=20)
    car.grid(row=0, column=0,padx=fenetreWidth/2-655)
