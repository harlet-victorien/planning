...
def setup2():
    ...
    Button(Co, text="Valider", command = verification).pack(side=RIGHT) #Bouton qui verifie que le mot de passe est bon
...
def create_day_boxes(parent,now):
    ...
    for i in range(nb_jours):
        ...
        button = Button(day_box, text="{} {}".format(day_name, i+1),font=('Arial',12),anchor="n", command=lambda day=i+1: show_day_events(day, events), background=color,width=12,height=5)
...
def eventsDetails(event):
    ...
    event_label = ttk.Label(event_details_window, text=event)
    event_label.pack()

def show_day_events(day, events):
    day-=1
    day_events_window = Toplevel(fenetre)
    day_events_window.title("Events for day {}".format(day))
    ...
    if len(events[day])>=1:
        for i in range(len(events[day])):
            event_label = ttk.Label(day_events_window, text=events[day][i])
...
if lancer1 :
    ...
    events = [[] for _ in range(31)]
    events[0] = ["Event1"]
    events[1] = ["Event2"]
    events[4] = ["Event2","fefr"]
    events[14] = ["Event 3","Event 3","Event 3",'dzgedzed']
    days_frame = ttk.Frame(fenetre, width=250, height=200)
    days_frame.grid(row=0, column=0, padx=fenetreHeight/2-100, pady=fenetreWidth/2-600)
    ...
    next_month_button.place(x=fenetreWidth-180,y=400)
    previous_month_button.place(x=60,y=400)
